...
int fit_pmt_wrapper(string rootFile, Int_t runID, Int_t fitID, Int_t runNum, Int_t daq, Int_t pedRate, Int_t dataRate, Int_t chan, Int_t amp, Int_t pmt, Int_t hv, Int_t ll, Int_t filter, Bool_t printSummary = false, Int_t constrainInj = 100, Int_t constrainGain = 0, Int_t constrainLL = 0, Bool_t saveResults = false, Bool_t saveNN = false, Int_t fitEngine = 0, Double_t g0 = 0.0, Double_t mu0 = 0.0){
	...
	Double_t hvMap[hvMapSize] = {2000, 1950, 1900, 1850, 1800, 1750, 1700, 1650, 1600, 1550};
	// Setup PMT 1
	gainLevels[1][0] = 6.1;
	gainLevels[1][1] = 5.6;
	gainLevels[1][2] = 5.1;
	...
	lightLevels[1][70] = 55.0;
	lightLevels[1][80] = 200.0;
	// Setup PMT 2
	gainLevels[2][0] = 6.1;
	gainLevels[2][1] = 5.6;
	gainLevels[2][2] = 5.1;
	...
	lightLevels[2][60] = 12.11;
	lightLevels[2][70] = 55.0;
	...
	Int_t low 		= getDataMinMax(rootFile, chan, 0, 150); // 150
	Int_t high		= getDataMinMax(rootFile, chan, 1, 15); // 15
	...
	// If amplified, affect change now
	if (amp > 1) sig0	= sig0 * amp;
	Double_t sigmin		= off;
	Double_t sigmax 	= off;
	...
	if (mumin >= 0 && mumax > 0) {
		int tempMinPE = (int)(mumin - 5);
		if (tempMinPE < 0) tempMinPE = 0;
		int tempMaxPE = (int)(mumax + 5);
	}
